...
def train(epoch, model, optimizer, scheduler, loss_function, train_loader,
    ...
    for step, (images, poses, gazes) in enumerate(train_loader):
        ...
        log['loss_train'].append(loss_meter.avg)
        loss_vis(log)
...
def validate(epoch, model, loss_function, val_loader, config,
    ...
    with torch.no_grad():
        for step, (images, poses, gazes) in enumerate(val_loader):
            ...
            l = len(log['loss_train']) - len(log['loss_val'])
            if len(log['loss_val']) > 0:
                log['loss_val'] = log['loss_val'] + [log['loss_val'][-1] for i in range(l)]
            else:
                log['loss_val'] = [log['loss_train'][0] for i in range(l)]
            log['loss_val'].append(loss_meter.avg)    
            loss_vis(log)
