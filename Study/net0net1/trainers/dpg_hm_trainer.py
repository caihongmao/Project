...
class dpg_hm_trainer(object):
    def __init__(self,
                 ...
                 start_epoch=1):
        super(dpg_hm_trainer, self).__init__()
    ...
    def train_step(self, inputs):
        ...
        center_hm_label = inputs['center_hm'].to(self.device)
        radius_hm_label = inputs['radius_hm'].to(self.device)

        gaze_pred, gmap_pred, _, _, center_hm_pred, radius_hm_pred, ldmks_pred, radius_pred = self.model(eye)
        ...
        loss_center_hm = self.compute_coord_loss(center_hm_label, center_hm_pred)* self.batch_size * 1000
        loss_radius_hm = self.compute_coord_loss(radius_hm_label, radius_hm_pred)* self.batch_size
        loss_radius = self.compute_coord_loss(radius_label, radius_pred)* self.batch_size 
        
        # loss = loss_center_hm * 0.05 + loss_radius_hm * 0.05
        loss = loss_radius  + loss_center_hm 
        ...
        loss.backward()
        self.optimizer.step()
        return loss.item(), loss_gmap.item(), loss_gaze.item(), loss_angel.item(), loss_center_hm.item(), loss_radius_hm.item(), loss_radius.item()
    ...
    def val_step(self, inputs):
        ...
        loss_gaze = self.model.dense_loss(gaze_pred, gaze_label)* self.batch_size
        loss_gmap = self.model.gmap_loss(gmap_pred, gmap_label) * 1e-5 * self.batch_size
        loss_center_hm = self.compute_coord_loss(center_hm_label, center_hm_pred)* self.batch_size
        loss_radius_hm = self.compute_coord_loss(radius_hm_label, radius_hm_pred)* self.batch_size
        loss_radius = self.compute_coord_loss(radius_label, radius_pred)* self.batch_size 
        ...
        # loss = loss_center_hm  * 0.05 + loss_radius_hm * 0.05
        loss = loss_radius  + loss_center_hm * 1000
    ...
    def train_epoch(self, dataset):
        ...
        for one_batch in dataset:
            batch_loss, loss_map, loss_gaze, loss_angel, loss_center_hm, loss_radius_hm, loss_radius = self.train_step(one_batch)
            ...
            if num_train_batches % 10 == 0:
                print('Trained batch:', num_train_batches,
                        ...
                        'Center hm loss', loss_center_hm,
                        'Radius hm loss', loss_radius_hm,
    ...
    def val_epoch(self, dataset):
        ...
        with torch.no_grad():
            ...
            num_val_batches = 0.0
            for one_batch in dataset:
                batch_loss, loss_map, loss_gaze, loss_angel, loss_center_hm, loss_radius_hm, loss_radius = self.val_step(one_batch)
                total_loss += batch_loss
                total_loss_gaze += loss_gaze
                num_val_batches += 1
    ...
    def run(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if val_loss < self.lowest_val_loss:
                if not os.path.exists(os.path.join('./models/dpghm')):
                    os.makedirs(os.path.join('./models/dpghm'))
                model_name = './models/dpghm/model-dpghm-{}-epoch-{}-loss-{:.4f}.pth'.format(self.version, epoch, val_loss_gaze)
                torch.save(self.model.state_dict(), model_name)
                plt.savefig('./models/dpghm/model-dpghm-{}-epoch-{}-loss-{:.4f}.png'.format(self.version, epoch, val_loss_gaze))
