...
class dpg_trainer(object):
    def __init__(self,
                 ...
                 start_epoch=1):
        super(dpg_trainer, self).__init__()
    ...
    def train_step(self, inputs):
        ...
        gaze_pred, gmap_pred, _, _= self.model(eye) * self.batch_size
        ...
        loss_angle = self.compute_angular_loss(gaze_pred, gaze_label)
        ...
        loss.backward()
        self.optimizer.step()
        return loss.item(), loss_gmap.item(), loss_angle.item()
    ...
    def val_step(self, inputs):
        ...
        gaze_pred, gmap_pred, _, _= self.model(eye)
        loss_gaze = self.model.dense_loss(gaze_pred, gaze_label) * self.batch_size
        loss_gmap = self.model.gmap_loss(gmap_pred, gmap_label) * 1e-5 * self.batch_size
        loss_angle = self.compute_angular_loss(gaze_pred, gaze_label)
    ...
    def train_epoch(self, dataset):
        ...
        for one_batch in dataset:
            batch_loss, loss_map, loss_angel = self.train_step(one_batch)
            ...
            if num_train_batches % 10 == 0:
                print('Trained batch:', num_train_batches,
                        ...
                        'Map loss', loss_map,
                        'Gaze loss', batch_loss - loss_map,
                        'Angel loss', loss_angel,
                        'Epoch total loss:', total_loss)
            if num_train_batches >= 15000 / self.batch_size:
                break
        return total_loss / num_train_batches, (total_loss - total_loss_map) / num_train_batches
    ...
    def val_epoch(self, dataset):
        ...
        with torch.no_grad():
            ...
            num_val_batches = 0.0
            for one_batch in dataset:
                batch_loss, loss_map, loss_angel = self.val_step(one_batch)
                total_loss += batch_loss
                total_loss_map += loss_map
                num_val_batches += 1
            ...
            loss_vis(self.log)
            return total_loss / num_val_batches,  (total_loss - total_loss_map) / num_val_batches
    ...
    def run(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if val_loss < self.lowest_val_loss:
                if not os.path.exists(os.path.join('./models/dpg')):
                    os.makedirs(os.path.join('./models/dpg'))
                model_name = './models/dpg/model-dpg-{}-epoch-{}-loss-{:.4f}.pth'.format(self.version, epoch, val_loss_gaze)
                torch.save(self.model.state_dict(), model_name)
                plt.savefig('./models/dpg/model-dpg-{}-epoch-{}-loss-{:.4f}.png'.format(self.version, epoch, val_loss_gaze))
