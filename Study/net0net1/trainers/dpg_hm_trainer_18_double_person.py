...
class dpg_hm_trainer_18(object):
    ...
    def train_step(self, inputs):
        ...
        gaze_pred, gmap_pred, heatmaps_pred, ldmks_pred, radius_pred = self.model(eye)

        loss_gaze = self.model.DPGHM18.dense_loss(gaze_pred, gaze_label)* self.batch_size
        loss_gmap = self.model.DPGHM18.gmap_loss(gmap_pred, gmap_label) * 1e-5 * self.batch_size
    ...
    def train_epoch(self, dataset):
        ...
        for one_batch in dataset:
            ...
            if num_train_batches >= 100 / self.batch_size:
                break
    ...
    def val_epoch(self, dataset):
        ...
        with torch.no_grad():
            ...
            for one_batch in dataset:
                ...
                if num_val_batches >= 20 / self.batch_size:
                    break
    ...
    def run(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if val_loss < self.lowest_val_loss:
                if not os.path.exists(os.path.join('./models/dpghm18double')):
                    os.makedirs(os.path.join('./models/dpghm18double'))
                model_name = './models/dpghm18double/model-dpghm18double-{}-epoch-{}-loss-{:.4f}.pth'.format(self.version, epoch, val_loss_gaze)
                torch.save(self.model.state_dict(), model_name)
                plt.savefig('./models/dpghm18double/model-dpghm18double-{}-epoch-{}-loss-{:.4f}.png'.format(self.version, epoch, val_loss_gaze))
