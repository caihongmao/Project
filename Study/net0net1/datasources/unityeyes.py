...
import utils.heatmap as heatmap_util
...
class UnityEyesDataset(torch.utils.data.IterableDataset):
    ...
    def gaussian_generate(self, shape, centre, radius):
        sigma = max(0.1, radius - 10)
        # xs col
    ...
    def preprocess_entry(self, entry):
        ...
        entry['gmap'] =  from_gaze2d(gaze, [self.eye_image_shape[0], self.eye_image_shape[1]])
        ...
        entry['center_hm'] = np.asarray([
                self.gaussian_generate(
                    shape=(self.heatmaps_scale*oh, self.heatmaps_scale*ow),
                    centre=self.heatmaps_scale*landmark,
                    radius=16,
                )
                for landmark in entry['landmarks'][-2:]
        ]).astype(np.float32)

        entry['radius_hm'] = np.asarray([
                self.gaussian_generate(
                    shape=(self.heatmaps_scale*oh, self.heatmaps_scale*ow),
                    centre=[30, 18],
